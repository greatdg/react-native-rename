...
var deletePreviousBundleDirectory = function deletePreviousBundleDirectory(_ref) {
  ...
  if (shouldDelete) {
    ...
    _shelljs2.default.rm('-rf', dir.replace('/main/java/com', '/debug/java/com'));
    ...
    var resolveJavaFiles = function resolveJavaFiles() {
      return new Promise(function (resolve) {
        readFile(_path2.default.join(__dirname, 'android/app/src/main/AndroidManifest.xml')).then(function (data) {
          ...
          var javaDebugFileBase = '/android/app/src/debug/java';

          var newJavaPath = javaFileBase + '/' + newBundleID.replace(/\./g, '/');
          var currentJavaPath = javaFileBase + '/' + currentBundleID.replace(/\./g, '/');

          var newDebugJavaPath = javaDebugFileBase + '/' + newBundleID.replace(/\./g, '/');
          var currentDebugJavaPath = javaDebugFileBase + '/' + currentBundleID.replace(/\./g, '/');
          ...
          var fullCurrentDebugBundlePath = _path2.default.join(__dirname, currentDebugJavaPath);
          var fullNewDebugBundlePath = _path2.default.join(__dirname, newDebugBundlePath);

          // Create new bundle folder if doesn't exist yet
          if (!_fs2.default.existsSync(fullNewDebugBundlePath)) {
            _shelljs2.default.mkdir('-p', fullNewDebugBundlePath);
            var _move = _shelljs2.default.exec('git mv "' + fullCurrentDebugBundlePath + '/"* "' + fullNewDebugBundlePath + '" 2>/dev/null');
            var _successMsg = newDebugBundlePath + ' ' + _colors2.default.green('BUNDLE INDENTIFIER CHANGED');

            if (_move.code === 0) {
              console.log(_successMsg);
            } else if (_move.code === 128) {
              // if "outside repository" error occured
              if (_shelljs2.default.mv('-f', fullCurrentDebugBundlePath + '/*', fullNewDebugBundlePath).code === 0) {
                console.log(_successMsg);
              } else {
          ...
          var vars = {
            ...
            newDebugBundlePath: newDebugBundlePath,
            javaDebugFileBase: javaDebugFileBase,
            currentDebugJavaPath: currentDebugJavaPath,
            newDebugJavaPath: newDebugJavaPath
          };
          resolve(vars);
        });
      });
    ...
    var resolveBundleIdentifiers = function resolveBundleIdentifiers(params) {
      return new Promise(function (resolve) {
        var currentBundleID = params.currentBundleID,
            ...
            newDebugBundlePath = params.newDebugBundlePath;


        (0, _bundleIdentifiers.bundleIdentifiers)(currentAppName, newName, projectName, currentBundleID, newBundleID, newBundlePath, newDebugBundlePath).map(function (file) {

          file.paths.map(function (filePath) {
            var newPaths = [];
        ...
        resolve({
          oldBundleNameDir: oldBundleNameDir,
